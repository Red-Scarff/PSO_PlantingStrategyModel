...
def predict_sale(x1, x2, coefficients, intercept):
    ...
    """
    # 构建特征列表，包括所有可能的四次项
    features = [1, x1, x2, x1**2, x1*x2, x2**2, x1**3, x1**2*x2, x1*x2**2, x2**3, x1**4, x1**3*x2,x1**2*x2**2, x1*x2**3, x2**4]
    # 计算y值
    y = intercept
...
def profit_function(x):
    ...
    for k in range(7):
        for row in range(num_rows):
            ...
            if flag: #大棚
                predict_scale = predict_sale(cost_new, price_new, coefficients_peng, intercept_peng)
                if predict_scale < 0: predict_scale = 0
                elif predict_scale > 20000: predict_scale = 18000
            else:
                predict_scale = predict_sale(cost_new, price_new, coefficients_field, intercept_field)
                if predict_scale < 0: predict_scale = 0
                elif predict_scale > 100000: predict_scale = 95000
            crop_areas = select_range(x, marks[row], j, crop_areas, k)
            production = crop_yield[row] * yield_change* crop_areas # 计算产量
            cost = crop_cost[row] * cost_change * crop_areas# 计算种植成本
            revenue = (
                min(production, predict_scale) * crop_price[row] * price_change
            )  # 计算有效收入(考虑滞销)
            total_profit += (
                revenue
                - cost
                + max(0, production - expected_demand[row]) * crop_price[row] / 4
            )  # 累加净收益
